...
export async function POST(request: Request) {
  ...
  try {
    ...
    const prompt = `
      ...
      {
        "question": "Soru metni burada olacak.",
        "choices": ["A seçeneği", "B seçeneği", "C seçeneği", "D seçeneği"],
        "correctAnswer": 0, // Doğru cevabın indeksi (0, 1, 2, veya 3)
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: 'Sen bir sınav sorusu oluşturucusun. Sadece geçerli JSON formatında yanıt ver.' },
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Soru verisi işlenirken hata oluştu.' },
        { status: 500 }
      );
    }
